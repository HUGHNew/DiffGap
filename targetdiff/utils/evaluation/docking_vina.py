...
class VinaDockingTask(BaseDockingTask):
    ...
    def __init__(self, protein_path, ligand_rdmol, tmp_dir='./tmp', center=None,
        ...
        ligand_rdmol = Chem.AddHs(ligand_rdmol, addCoords=True) # Mol with coordinate

        sdf_writer = Chem.SDWriter(self.ligand_path) # Mol2Sdf
        sdf_writer.write(ligand_rdmol) # save sdf file
        sdf_writer.close()
    ...
    def run(self, mode='dock', exhaustiveness=8, **kwargs):
        ligand_pdbqt = self.ligand_path[:-4] + '.pdbqt' # .sdf -> .pdbqt ligand prepare
        protein_pqr = self.receptor_path[:-4] + '.pqr' # .pdb -> .pqr protein prepare
        protein_pdbqt = self.receptor_path[:-4] + '.pdbqt' # .pdb -> .pdbqt protein prepare (the same dir as ligand)
