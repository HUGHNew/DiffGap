    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        for sample_idx, (pred_pos, pred_v) in enumerate(zip(all_pred_ligand_pos, all_pred_ligand_v)):
            ...
            try:
                ...
                if args.verbose:
                    logger.warning(f"Evaluation failed for {example_idx}_{sample_idx}: {e}")
                continue
        ...
        pro_fname = r["data"].protein_filename.split('/')[0]
        logger.info(f"Evaluate [{example_idx}][{pro_fname}]: {duration}s={misc.convert_seconds(duration)}")
