...
def main(args:TrainArgs) -> str:
    # Load configs
    cfg = misc.load_config(args.config, False)
    ...
    logger.info(args)
    logger.info(config)
    misc.dump_config(cfg, os.path.join(log_dir, os.path.basename(args.config))) # save config file
    ...
    try:
        ...
        last_save, max_interval = None, 50_000
        ...
        logger.info("Signal received. Saving current status before terminate.")
    torch.save({
        'config': config,
        'model': model.state_dict(),
        'optimizer': optimizer.state_dict(),
