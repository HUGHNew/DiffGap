...
from utils.misc import method_resolve, METHOD_PSP, METHOD_SP
...
def log_sample_categorical(logits):
    ...
    # reference: https://en.wikipedia.org/wiki/Categorical_distribution
    # X = \mu - \beta \cdot (-ln(-ln(U)));
    # \mu=0, positional parameter
...
# %%
# endregion operation in log-space
...
class ScorePosNet3D(nn.Module):
    ...
    def get_perturb_pos_v(self, ligand_pos, ligand_v, batch_ligand, time_step):
        ...
        return ligand_pos_perturbed, log_ligand_v0, ligand_v_perturbed, log_ligand_vt, pos_noise

    def get_prediction_p_v_n(
        self,
        ...
        pred_pos_noise = pred_ligand_pos - init_ligand_pos

        return pred_ligand_pos, pred_ligand_v, pred_pos_noise

    def get_diffusion_loss(
            self, protein_pos, protein_v, batch_protein, ligand_pos, ligand_v, batch_ligand, time_step=None, bias=None
    ):
        ...
        if time_step is None:
            time_step, pt = self.sample_time(num_graphs, protein_pos.device, self.sample_time_method) # (num_graphs, )
        else:
            pt = torch.ones_like(time_step).float() / self.num_timesteps
        ...
        (
            ...
            log_ligand_vt,
            pos_noise,
        ) = self.get_perturb_pos_v(ligand_pos, ligand_v, batch_ligand, time_step)

        # EB config && t+1 exists && 1-p
        entry_controller = \
            bias != None and \
            not (time_step==self.num_timesteps-1).any() and \
            torch.rand((1,)) > bias.p_value

        if entry_controller:
            ######################
            # For t+1, we should make t < num_timesteps-1.
            # 2.1. Rationale for `time_step==self.num_timesteps` filter
            # The P(time_step==self.num_timesteps-1|time_step = rand(0, self.num_timesteps)) is 1/self.num_timesteps.
            # As self.num_timesteps equals 1K, P equals 1/1000.
            # Therefore, the P(not filter) = 0.999 ^ N. Looks acceptable.
            # max(N) = 4 ~ avg(N) for training. P(N=4)=0.996006

            # 2.2. perturb pos and v with t+1
            time_step_next = time_step + 1
            (
                ligand_pos_perturbed_t1,
                log_ligand_v0_t1,
                ligand_v_perturbed_t1,
                log_ligand_vt1,
                pos_noise_t1,
            ) = self.get_perturb_pos_v(ligand_pos, ligand_v, batch_ligand, time_step_next) # x_{t+1}

            bias_method = method_resolve(bias.method)

            # if bias.method in ["PSP"]:
            if bias_method > METHOD_SP:
                # [2.3.] recover \hat x_t
                # [pos,v] = \hat x_0
                hat_ligand_pos, hat_ligand_v, hat_pos_noise = self.get_prediction_p_v_n(
                    protein_pos=protein_pos,
                    protein_v=protein_v,
                    batch_protein=batch_protein,
                    init_ligand_pos=ligand_pos_perturbed_t1,
                    init_ligand_v=ligand_v_perturbed_t1,
                    batch_ligand=batch_ligand,
                    time_step=time_step_next,
                )
                # hat_ligand_v: [Elems , self.num_classes]
                # ligand_v: [Elems]
                ...
                if self.model_mean_type == "C0":
                    ...
                    (
                        ...
                        log_ligand_vt1,
                        pos_noise_t1,
                    ) = self.get_perturb_pos_v(hat_ligand_pos, hat_ligand_v.argmax(dim=-1), batch_ligand, time_step)
                elif self.model_mean_type == "noise":
                    # pos: Gaussian noise; v: Uniform noise

                    # get predicted \hat pos_t
                    sigma_next = torch.sqrt(extract(self.betas, time_step_next, batch_ligand)) # (num_ligand_atoms, 1)
                    sigma_z = sigma_next * torch.normal(0, 1, sigma_next.shape, device=sigma_next.device) # (num_ligand_atoms, 1)
                    one_minus_alpha_cof = extract(self.sqrt_one_minus_alphas_cumprod, time_step_next, batch_ligand) # (num_ligand_atoms, 1)
                    alpha_cof = extract(self.sqrt_alphas_cumprod, time_step_next, batch_ligand) # (num_ligand_atoms, 1)
                    # \hat x_t=(x_t-\sqrt{1-\bar\alpha_{t+1}}\epsilon_\theta(x_{t+1},{t+1}))/\sqrt{\bar\alpha_{t+1}}+\sigma_{t+1}z; \sigma_{t+1}=\beta_{t+1}
                    ligand_pos_perturbed_t1 = (ligand_pos_perturbed - one_minus_alpha_cof * hat_ligand_pos) / alpha_cof + sigma_z # (num_ligand_atoms, 3)

                    # get predicted \hat v_t
                    # Vt from V0
                    # hat_ligand_v
                    ligand_v_perturbed_t1 = self.q_v_posterior(log_ligand_v0, log_ligand_vt1, time_step_next, batch_ligand) # Predict v0
                    # hat_log_ligand_vt
                    log_ligand_vt1 = index_to_log_onehot(hat_ligand_v.argmax(dim=-1), self.num_classes)
                else:
                    raise ValueError

            # 2.6. replace the outer variables
            # default_method = SP
            (
                ligand_pos_perturbed,
                log_ligand_v0,
        ...
        pred_ligand_pos, pred_ligand_v, pred_pos_noise = self.get_prediction_p_v_n(
            protein_pos=protein_pos,
            protein_v=protein_v,
            batch_protein=batch_protein,
